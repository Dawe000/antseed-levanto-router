 ...
 * router-driven savings from marketplace-driven savings. The endpoint must
 * serve the OpenRouter-compatible models schema -- `{ data: [{ id, name,
 * pricing: { prompt, completion, input_cache_read } }] }` with prices in
 * USD per token -- which is what the OpenRouter* type names in this file
 * refer to. The URL comes from the
