...
describe('BuyerPaymentManager', () => {
  ...
  describe('signCumulativeAuth (model-routing flat-fee day pass)', () => {
    ...
    it('retiring and reopening a session for the same seller does not reset the elapsed-day clock', async () => {
      // Regression: a seller that closes the channel right after accepting a
      // signature (cooperatively, or via a genuine exhaustion/top-up-failure
      // path) used to force an immediate extra day's charge on the reopened
      // channel, since _lastFlatFeeSignedAt was wiped on every retirement.
