 ...
 * callback -- the routing plugin never holds a signing key directly (that
 * would let plugin code, including third-party routers, sign arbitrary
 * messages), so the host builds the actual signing closure from the
 * buyer's real `Identity` and hands it to whichever router requests one.
 *
