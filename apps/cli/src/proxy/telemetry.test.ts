...
test('attachStreamingAntseedHeaders attaches provider and service, matching the non-streaming path', () => {
...
test('attachStreamingAntseedHeaders reports the resolved model, not the auto-router sentinel, for a routed request', () => {
  // The caller passes the already-substituted request (withRoutedModel), not
  // the original "auto-router" sentinel -- this test locks that in.
  const headers = attachStreamingAntseedHeaders({}, peer(['openai']), 'req-1', req('gpt-5.6-luna'))
  assert.notEqual(headers['x-antseed-service'], 'auto-router')
})
