...
export async function fetchProxyConversationRoute(
  ...
  try {
    ...
    // Report the failure here so a timeout (2s AbortSignal), a network
    // error, or a JSON parse failure is distinguishable from "the proxy
    // genuinely has nothing to report yet" -- both would otherwise fall
    // back to the raw SDK's own meta with no peerId/routeAlternatives and
    // no trace of why.
    onError?.(
