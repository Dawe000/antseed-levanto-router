...
test('ensureConfig preserves cqt and dayPassOnDemandEnabled even when the migration touches other routing preferences', async (t) => {
  ...
  // mentioned cqt/dayPassOnDemandEnabled -- silently dropping both. In
  // practice this meant a user's Auto toggle (dayPassOnDemandEnabled)
  ...
  assert.equal(prefs.dayPassOnDemandEnabled, true, 'the Auto toggle must survive a migration triggered by an unrelated field');
});
